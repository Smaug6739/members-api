...
export class MemberClass {
    ...
    public put(
        ...
        return new Promise<IObject | Error>((resolve, reject) => {
            ...
            hash(password, 10)
                .then((hash: string) => {
                    db.query('INSERT INTO members (`nickname`, `permissions`, `banishment`, `avatar`, `password`, `first_name`, `last_name`, `age`, `phone_number`, `email`, `date_insert`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);', [nickname, permissions, banichement, avatar, hash, firstName, lastName, age, phoneNumber, email], (err: any, result: Array<IObject>): void => {
                        if (err) return reject(new Error(err))
                        return resolve(result)
                    })
                })
